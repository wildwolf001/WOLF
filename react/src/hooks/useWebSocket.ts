...
import type { WSServerEvents } from '@/types/api';
...
export function useWebSocket() {
  ...
  const sendMessage = useCallback((agentId: string, content: string, history?: Array<{ role: string; content: string }>) => {
    setTyping(true);
    wsService.sendToAgent(agentId, content, history);
  }, [setTyping]);
...
export function useWebSocketEvent<K extends keyof WSServerEvents>(
  event: K,
  handler: (data: WSServerEvents[K]) => void
) {
  useEffect(() => {
    const unsubscribe = wsService.subscribe(event, handler as (data: unknown) => void);
    return unsubscribe;
  }, [event, handler]);
}